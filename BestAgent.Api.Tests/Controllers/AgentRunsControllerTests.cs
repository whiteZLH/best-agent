...
public class AgentRunsControllerTests
{
    ...
    public async Task GetSteps()
    {
        ...
        var mediator = new FakeMediator((GetAgentRunStepsQuery query) =>
            ...
            [
                new(
                    ...
                    null,
                    "plan",
                    new HandoffInfo("handoff", "support_agent", "handoff-input", "delegate_and_wait", "child-run-1", "Approved", "Completed", "child-output", null, now, "route-rule-1", "{\"mode\":\"summary_only\"}", "{\"mode\":\"read_only\"}", "{\"inherit\":false}", "{\"sources\":[\"faq\"]}", true, "Route to refund specialist", 0.91, "{\"mode\":\"summary_only\"}", "{\"mode\":\"read_only\"}", "{\"allowed\":[\"faq_search\"]}"),
        ...
        Assert.Equal("delegate_and_wait", step.Handoff.Mode);
