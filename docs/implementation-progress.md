...
- 可观测性：当前已补上最小 Metrics 闭环，`IAgentMetrics` + `System.Diagnostics.Metrics` 已开始覆盖 Run 创建/终态、模型调用耗时与 token/cost、工具执行耗时，以及审批等待/超时时长；同时也已开始用统一 `ActivitySource` 为 `AgentRun`、`ModelCall`、`ToolExecution`、`Approval` 与 `Handoff` 链路补最小 tracing span，但 Retrieval span、日志分层、跨实例 exporter / dashboard、trace 传播与更细粒度经营指标仍未完整落地
