...
- OpenAI 兼容模型网关当前也已开始支持消费原生单个 `tool_calls` / `toolCalls` 与 legacy `function_call` / `functionCall` 响应，并统一归一回现有 `{"action":"tool_call",...}` JSON 决策输出；其中 `arguments` 当前既可兼容字符串，也可兼容部分网关直接返回的 JSON 对象形态；兼容网关若把这组原生工具调用直接挂在 `choice` 层，当前也已开始继续解析
