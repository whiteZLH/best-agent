...
- OpenAI 兼容模型网关当前也已开始兼容响应中 `message.content`、`message.output_text/outputText`、`choice.content`、`choice.text` 与 `choice.output_text/outputText` 的字符串、数组文本片段与最小对象文本形态，不再只依赖单个字符串 `content`；其中即使兼容响应未提供 `message` 对象、只在 `choice` 层直接返回正文，当前也会继续提取；对常见的嵌套文本对象（如 `text.value`、`output_text`、`outputText`）当前也已开始做最小兼容解析，`reasoning_summary/reasoning` 会沿用同一套文本提取逻辑，并额外兼容 `reasoningSummary`；若兼容响应把最终拒答文本放在 `message.refusal` 或 `choice.refusal`，当前也会继续提取，而不再直接报空响应
...
- OpenAI 兼容模型网关当前也已开始在非 `2xx` 响应时提取最小结构化错误消息：若兼容网关返回 OpenAI 风格 `error.message`、`error.title/error_description`、顶层 `message/detail/description/error_description/errors[]`，或常见校验错误数组中的 `detail[].msg`，异常文本会优先带上这类归一后的错误信息，而不再只拼接整段原始 body；同时当更靠前的候选字段存在但为空时，当前也会继续回退后续候选字段，而不再被空 `message` 等占位字段阻断
