...
- OpenAI 兼容模型网关当前也已开始把 OpenAI 兼容响应中的 `finish_reason` 规范化为平台侧最小枚举（如 `completed` / `tool_call` / `max_output_tokens` / `content_filtered`），其中 `tool_calls` / `tool_call`、`length` / `max_output_tokens`、`content_filter` / `content_filtered` 当前都会统一归一到平台侧最小枚举，同时也已开始兼容 camelCase 形式的 `finishReason` 字段与常见 camelCase 值（如 `toolCalls`、`toolCall`、`functionCall`、`maxOutputTokens`、`contentFilter`、`contentFiltered`）；若兼容响应未显式返回 `finish_reason/finishReason`，但消息体或 `choice` 层已带原生 `tool_calls/toolCalls` 或 legacy `function_call/functionCall`，当前也会最小推断为 `tool_call`；同时把响应 `id` 与实际 `service_tier` 一并写入 `GenerateTextResult` 与 `model_call` 审计 payload，其中 `response id` 当前也已开始兼容 `id`、`response_id`、`request_id`、`responseId` 与 `requestId`，`service_tier` 也已开始兼容 `serviceTier`；`usage` 里的 token 统计则额外兼容 `promptTokens/inputTokens/completionTokens/outputTokens/totalTokens`，并在 `usage` 对象缺失时回退读取响应顶层的同名平铺字段
- `GenerateTextRequest` 当前也已开始支持最小 `Tools` 输入模型；`AgentRunLoop` 会按版本级 `AllowedTools` 解析 enabled `ToolDefinition`，并向 OpenAI 兼容请求下发最小 `tools.function.parameters`，同时当前也会对工具定义显式补上 `function.strict`；请求侧若显式传入 `Tools`，当前要求至少存在一个具名工具定义，空列表会直接拒绝，具名工具定义的名字也要求大小写不敏感唯一，且需满足 OpenAI 兼容命名约束（字母/数字/下划线/短横线，最长 `64` 字符）；`tool_choice=auto|required|具体工具名` 也要求至少存在有效声明工具
- `GenerateTextRequest` 当前也已开始支持最小 `Messages` 输入模型；显式传入消息列表时，OpenAI 兼容请求会优先按多消息形式下发，当前支持 `developer/system/user/assistant/tool` 角色，单条消息也已开始支持最小 `name` / `tool_call_id` 字段，其中 `tool` 角色消息要求显式携带 `tool_call_id`；同时消息 `content` 当前也已开始支持字符串与最小结构化 content parts（`text` / `image_url` / `input_audio` / `file`），`assistant` 角色消息也已开始支持显式 `tool_calls` 历史载荷，且其中历史 tool call 名称当前也已开始收紧到 OpenAI 兼容命名约束；对显式 `Messages` 中的空列表或无效项，网关当前都会直接拒绝，不再静默丢弃坏消息或回退到 `system prompt + input`
- OpenAI 兼容模型网关当前也已开始兼容响应中 `message.content`、`message.output_text/outputText`、`choice.text` 与 `choice.output_text/outputText` 的字符串、数组文本片段与最小对象文本形态，不再只依赖单个字符串 `content`；其中即使兼容响应未提供 `message` 对象、只在 `choice` 层直接返回正文，当前也会继续提取；对常见的嵌套文本对象（如 `text.value`、`output_text`、`outputText`）当前也已开始做最小兼容解析，`reasoning_summary/reasoning` 会沿用同一套文本提取逻辑，并额外兼容 `reasoningSummary`；若兼容响应把最终拒答文本放在 `message.refusal` 或 `choice.refusal`，当前也会继续提取，而不再直接报空响应
- OpenAI 兼容模型网关当前也已开始支持消费原生单个 `tool_calls` / `toolCalls` 与 legacy `function_call` / `functionCall` 响应，并统一归一回现有 `{"action":"tool_call",...}` JSON 决策输出；其中 `arguments` 当前既可兼容字符串，也可兼容部分网关直接返回的 JSON 对象形态；兼容网关若把这组原生工具调用直接挂在 `choice` 层，当前也已开始继续解析；若同层同时返回空 `tool_calls` 数组与有效 `function_call`，当前也会继续回退解析后者
- OpenAI 兼容模型网关当前也已开始在非 `2xx` 响应时提取最小结构化错误消息：若兼容网关返回 OpenAI 风格 `error.message`、`error.title/error_description`、顶层 `message/detail`，或常见校验错误数组中的 `detail[].msg`，异常文本会优先带上这类归一后的错误信息，而不再只拼接整段原始 body；同时当更靠前的候选字段存在但为空时，当前也会继续回退后续候选字段，而不再被空 `message` 等占位字段阻断
...
- OpenAI 兼容模型网关当前也已开始对原生 `tool_calls` 做最小结构校验：返回的工具名必须命中当前声明工具列表，`arguments` 也必须是可解析的 JSON 对象；兼容网关若直接返回对象而不是 JSON 字符串，当前也会先归一后再继续校验
