...
- OpenAI 兼容模型网关当前也已开始在非 `2xx` 响应时提取最小结构化错误消息：若兼容网关返回 OpenAI 风格 `error.message`、`error.title/error_description`、顶层 `message/detail/description/error_description`，或常见校验错误数组中的 `detail[].msg`，异常文本会优先带上这类归一后的错误信息，而不再只拼接整段原始 body；同时当更靠前的候选字段存在但为空时，当前也会继续回退后续候选字段，而不再被空 `message` 等占位字段阻断
