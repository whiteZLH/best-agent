...
- OpenAI 兼容模型网关当前也已开始兼容响应中 `message.content`、`choice.text` 的字符串、数组文本片段与最小对象文本形态，不再只依赖单个字符串 `content`；其中对常见的嵌套文本对象（如 `text.value`、`output_text`、`outputText`）当前也已开始做最小兼容解析，`reasoning_summary/reasoning` 会沿用同一套文本提取逻辑，并额外兼容 `reasoningSummary`；若兼容响应把最终拒答文本放在 `message.refusal` 或 `choice.refusal`，当前也会继续提取，而不再直接报空响应
