...
- OpenAI 兼容模型网关当前也已开始把 OpenAI 兼容响应中的 `finish_reason` 规范化为平台侧最小枚举（如 `completed` / `tool_call` / `max_output_tokens` / `content_filtered`），其中 `tool_calls` / `tool_call`、`length` / `max_output_tokens`、`content_filter` / `content_filtered` 当前都会统一归一到平台侧最小枚举；若兼容响应未显式返回 `finish_reason`，但消息体已带原生 `tool_calls` 或 legacy `function_call`，当前也会最小推断为 `tool_call`；同时把响应 `id` 与实际 `service_tier` 一并写入 `GenerateTextResult` 与 `model_call` 审计 payload，其中 `response id` 当前也已开始兼容 `id`、`response_id`、`request_id`、`responseId` 与 `requestId`，`service_tier` 也已开始兼容 `serviceTier`；`usage` 里的 token 统计则额外兼容 `promptTokens/inputTokens/completionTokens/outputTokens/totalTokens`
...
- OpenAI 兼容模型网关当前也已开始兼容响应中 `message.content` 的字符串、数组文本片段与最小对象文本形态，不再只依赖单个字符串 `content`；其中对常见的嵌套文本对象（如 `text.value`、`output_text`）当前也已开始做最小兼容解析，`reasoning_summary/reasoning` 会沿用同一套文本提取逻辑；若兼容响应把最终拒答文本放在 `message.refusal` 或 `choice.refusal`，当前也会继续提取，而不再直接报空响应
- OpenAI 兼容模型网关当前也已开始支持消费原生单个 `tool_calls` 与 legacy `function_call` 响应，并统一归一回现有 `{"action":"tool_call",...}` JSON 决策输出；其中 `arguments` 当前既可兼容字符串，也可兼容部分网关直接返回的 JSON 对象形态
