...
> - 当前已支持最小子 Agent 执行与父子 Run 协同，并已开始支持单子 Run 的最小结果合并逻辑；但还没有多 Agent / 多结果汇聚逻辑。
...
- Planner 产出的 `context_overrides` / `tool_overrides` / `knowledge_overrides` 当前也会进入 handoff 审计 payload
