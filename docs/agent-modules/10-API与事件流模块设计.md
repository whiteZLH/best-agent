...
> - 当前也已开始支持最小强制鉴权开关：当 `Authentication:RequireAuthenticatedRunAccess=true` 时，交互式 Run API 要求 bearer 身份后方可访问；外部 `tool/approval complete` 回调继续沿用 HMAC 校验，不受该开关影响。
> - 当前管理接口也已开始支持最小强制鉴权开关：当 `Authentication:RequireAuthenticatedManagementAccess=true` 时，`AgentDefinitionsController` / `ToolDefinitionsController` 入口要求 bearer 身份后方可访问；若进一步配置 `Authentication:ManagementAllowedRoles`，则还会执行最小角色门禁。
...
- 当前已开始补充最小扩展 options 模型：
  ...
  - `options.stream` 已进入请求契约层，并已开始支持最小 discoverability：创建 Run 时显式传入 `true` 会在响应中直接返回对应的 `streamUrl`；实际流式消费仍通过 `GET /agent-runs/{runId}/stream`
...
- 当前分别返回：
  - Run 快照
    - `GET /agent-runs/{runId}` 当前也已开始补充最小 `streamUrl`
  - 子 Run 列表 / Run Tree
    - `GET /agent-runs/{runId}/children` 与 `GET /agent-runs/{runId}/tree` 当前也已开始为各节点补充最小 `streamUrl`
  - Step 列表
  - 审批列表
...
- 当 `Authentication:RequireAuthenticatedManagementAccess=true` 时，`GET /tool-definitions*`、`POST/PUT/DELETE /tool-definitions*`、`GET /agent-definitions*`、`POST /agent-definitions*`、route-rule 管理接口与 `:activate-version` 入口都会要求已认证身份；若同时配置 `Authentication:ManagementAllowedRoles`，这些入口还会拒绝不在允许角色集合内的已认证身份
