...
> - 当前管理接口也已开始支持最小强制鉴权开关：当 `Authentication:RequireAuthenticatedManagementAccess=true` 时，`AgentDefinitionsController` / `ToolDefinitionsController` 入口要求 bearer 身份后方可访问。
...
- 当 `Authentication:RequireAuthenticatedManagementAccess=true` 时，`GET /tool-definitions*`、`POST/PUT/DELETE /tool-definitions*`、`GET /agent-definitions*`、`POST /agent-definitions*`、route-rule 管理接口与 `:activate-version` 入口都会要求已认证身份
