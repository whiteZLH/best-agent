...
- 当前事件 `Data` 读侧也已开始补充 `Approval` / `Handoff` / `HumanWait` 结构化视图；事件 payload 内的工具输入 / 输出字段继续按既有最小递归脱敏策略返回
...
- `GET /tool-definitions*` 当前也已落地，并开始同时返回两层工具执行定义视图：
  ...
  - 新的结构化 `Execution` 读侧：显式区分 `webhook`、`local_handler`、`inline_result`
...
- `POST/PUT /tool-definitions*` 当前也已开始同时兼容两层写入视图：
  ...
  - 新的结构化 `Execution` 写侧：可直接提交 `webhook`、`local_handler`、`inline_result` 执行定义
...
- 当前 SSE 实时通道也已开始补齐最小统一事件信封：
  ...
  - typed `data` 当前除 `ModelCall` 外，也已同步回显 `Approval` / `Handoff` / `HumanWait`
